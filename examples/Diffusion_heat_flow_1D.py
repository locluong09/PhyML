...
class NeuralNetTorch(nn.Module):
    def __init__(self, layer_sizes):
        ...
        Args:
            layer_sizes: A list of integers defining the number of neurons in each layer,
                         including the input and output layers.
                         e.g., [1, 10, 1] represents a network with 1 input, 10 hidden units, and 1 output.
        """
    ...
    def forward(self, coord):
        ...
        Args:
            coord: Input coordinates (PyTorch Tensor of shape Nx1).

        Returns:
            h_nn: Output of the neural network (PyTorch Tensor).
        """
        h_nn = coord
        for i, layer in enumerate(self.layers):
            h_nn = layer(h_nn)
            if i < len(self.layers) - 1:  # Apply tanh to hidden layers
                h_nn = torch.tanh(h_nn)
        return h_nn
...
def pde_residual_1d_darcy(spatial, time, model):
    ...
    pde_residual = h_xx + 10*torch.cos(x)
...
if __name__ == "__main__":
    ...
    # Boundary conditions
    lb_head = 0.0  # Head at left boundary
    rb_head = 0.0  # Head at right boundary
    ...
    net = NeuralNetTorch(layers).to(device)
    ...
    h_analytical = 10.0*((torch.cos(x_plot)-1) + x_plot*(1-torch.cos(torch.tensor(1.0))))
